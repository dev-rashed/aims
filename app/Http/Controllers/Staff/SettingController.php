...
use App\Http\Requests\Staff\SettingRequest;
...
class SettingController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('staff.setting.edit', compact('data'));
    }
    ...
    {
        $inputs = $request->except('_token', '_method', 'mail', 'social_name', 'social_url', 'stripe', 'google', 'vimeo');

        $inputs['logo'] = $request->hasFile('logo') ? fileUpload($request->file('logo'), 'setting', setting('logo')) : setting('logo');
        $inputs['dark_logo'] = $request->hasFile('dark_logo') ? fileUpload($request->file('dark_logo'), 'setting', setting('dark_logo')) : setting('dark_logo');
        $inputs['footer_logo'] = $request->hasFile('footer_logo') ? fileUpload($request->file('footer_logo'), 'setting', setting('footer_logo')) : setting('footer_logo');
        $inputs['favicon'] = $request->hasFile('favicon') ? fileUpload($request->file('favicon'), 'setting', setting('favicon')) : setting('favicon');
        $inputs['default_image'] = $request->hasFile('default_image') ? fileUpload($request->file('default_image'), 'setting', setting('default_image')) : setting('default_image');


        $socials = [];
        foreach ($request->social_name as $key => $social_name) {
            $socials[$key] = [
                'name' => $social_name,
                'url' => $request->social_url[$key],
            ];
        }
        $inputs['socials'] = json_encode($socials);
        ...
        if (file_exists($path)) {
            ...
            $envs = [
                ...
                'MAIL_MAILER' => $request->mail['mailer'],
                ...
                'MAIL_FROM_ADDRESS' => $request->mail['from_address'],
                'MAIL_FROM_NAME' => $request->mail['from_name'],

                'STRIPE_KEY' => $request->stripe['key'],
                'STRIPE_SECRET' => $request->stripe['secret'],
                'STRIPE_ENABLE' => $request->stripe['enable'],

                'GOOGLE_CLIENT_ID' => $request->google['client_id'],
                'GOOGLE_CLIENT_SECRET' => $request->google['client_secret'],
                'GOOGLE_MAPS_API_KEY' => $request->google['map_api_key'],
                'RECAPTCHA_SITE_KEY' => $request->google['recaptcha_site_key'],
                'RECAPTCHA_SECRET_KEY' => $request->google['recaptcha_secret_key'],
                'RECAPTCHA_ENABLE' => $request->google['recaptcha_enable'],

                'VIMEO_CLIENT' => $request->vimeo['client_id'],
                'VIMEO_SECRET' => $request->vimeo['client_secret'],
                'VIMEO_ACCESS' => $request->vimeo['access_token'],

                'TINYMCE_API_KEY' => $request->tinymce_api_key,
            ];
        ...
        return response()->json(['message' => translate('updated_message', ['text' => 'Setting'])]);
    }
}
