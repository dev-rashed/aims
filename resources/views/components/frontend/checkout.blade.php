...
<!-- About Section Start -->
{{-- <section class="section p-0 bg-s-cover bg-r-no" style="background-image: url({{ asset('build/assets/frontend/images/membership-bg.svg') }});height:400px;">
    <div class="d-flex h-100 justify-content-center alig-items-center">
        <h1 class="fs-1 fw-bold text-primary-900 align-self-center">{{ $title }}</h1>
    </div>
</section> --}}
<!-- About Section End -->
            ...
            <div class="col-lg-10 mx-lg-auto">
                <div class="card rounded-20 border-0 shadow-lg">
                    <div class="card-body py-5">
                        <form id="submit" action="{{ $route }}" method="POST" enctype="multipart/form-data" data-redirect="{{ route('checkout.confirmation') }}">
                            @csrf

                            <input type="hidden" name="{{ $from }}" value="{{ $product['slug'] }}" />

                            <h1 class="fs-1 fw-bold text-primary-500 text-center">Booking</h1>
                            ...
                            <div class="tab-content" id="pills-tabContent">
                                ...
                                <div class="tab-pane fade show active" id="pills-1">
                                    ...
                                    <div class="row mt-5">
                                        ...
                                        <x-frontend.input-group label="phone_number" type="tel" for="phone" :value="auth()->user()->phone ?? ''" column="col-lg-6" />
                                            ...
                                            <div class="text-gray-600 fs-16 fw-medium">
                                                <div class="border-bottom-1 border-gray-300 mb-2 pb-3">
                                                    ...
                                                    <div class="d-flex justify-content-between">
                                                        ...
                                                        <p class="mb-0">{{ convertAmount($product['price']) }}</p>
                                                    </div>
                                                </div>
                                                <div class="d-flex justify-content-between">
                                                    <p class="mb-0">Subtotal</p>
                                                ...
                                                <div class="d-flex justify-content-between">
                                                    ...
                                                    <p class="mb-0" id="total_amount">{{ convertAmount($product['price']) }}</p>
                                            ...
                                            <div class="row h-100 gy-5">
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    <div class="d-flex position-relative">
                                                        ...
                                                        <input type="text" id="code" class="form-control border-primary-500 padding-right" placeholder="Promo code" @readonly(intVal($product['price']) <= 0) />
                                                        <button type="button" @class(['btn bg-primary-500 rounded-10 px-5 py-2 text-white ms-2', 'disabled' => intVal($product['price']) <= 0]) id="applyCheckoutCoupon" data-url="{{ route('coupon.store') }}" data-from="{{ $from }}">Apply</button>
                                                    </div>
                                                </div>

                                                @if (config('services.stripe.enable') && intVal($product['price']) > 0)
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-5">
                                ...
                                <button type="button" id="nextBtn" @class(['btn bg-primary-500 text-white rounded-10 px-5', 'disabled' => empty(auth()->user()->first_name) || empty(auth()->user()->last_name) || empty(auth()->user()->email) || empty(auth()->user()->phone) || empty(auth()->user()->location)])>Continue</button>
                            </div>
                        </form>
                    </div>
                </div>
